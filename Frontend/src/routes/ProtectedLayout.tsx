...
export function ProtectedLayout() {
  const { session, loading, roleId, roleName, displayName, otpVerified, signOut } =
    useAuth()
        ...
        <nav>
          {roleId === RoleId.Customer && (
            <>
              ...
              <NavLink to="/my-policy" className={({ isActive }) => (isActive ? 'active' : '')}>
                <FileText size={17} /> My Policy
              </NavLink>
              <NavLink to="/my-vehicle" className={({ isActive }) => (isActive ? 'active' : '')}>
                <Car size={17} /> My Vehicle
              </NavLink>
              <NavLink to="/my-claims/new" className={({ isActive }) => (isActive ? 'active' : '')}>
                <FilePlus2 size={17} /> Raise Claim
              </NavLink>
              <NavLink to="/my-claims" end className={({ isActive }) => (isActive ? 'active' : '')}>
                <ClipboardList size={17} /> My Claims
              </NavLink>
            </>
          )}
          ...
          {roleId === RoleId.Admin && (
            <>
              ...
              <NavLink to="/admin/claims" className={({ isActive }) => (isActive ? 'active' : '')}>
                <ClipboardList size={17} /> All Claims
              </NavLink>
              ...
              >
                <SlidersHorizontal size={17} /> Authority Limits
              </NavLink>
              <NavLink
                to="/admin/scoring-rules"
                className={({ isActive }) => (isActive ? 'active' : '')}
              >
                <SlidersHorizontal size={17} /> Scoring Rules
              </NavLink>
            </>
          )}
        </nav>
